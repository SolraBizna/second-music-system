...
struct ForeignSoundReader<T: Sample> {
    ...
    read_handler:
        unsafe extern "C" fn(*mut c_void, *mut c_void, size_t) -> size_t,
    free_handler: Option<unsafe extern "C" fn(*mut c_void)>,
    seek_handler: Option<unsafe extern "C" fn(*mut c_void, u64) -> u64>,
    skip_precise_handler: Option<
        unsafe extern "C" fn(*mut c_void, u64, *mut c_void, size_t) -> c_int,
    >,
    skip_coarse_handler: Option<
        unsafe extern "C" fn(*mut c_void, u64, *mut c_void, size_t) -> u64,
    >,
    clone_handler: Option<
        unsafe extern "C" fn(
            *mut c_void,
            f32,
            c_int,
...
impl<T: Sample> Drop for ForeignSoundReader<T> {
    fn drop(&mut self) {
        if let Some(free_handler) = self.free_handler {
            unsafe {
                free_handler(self.callback_data);
            }
        }
    }
}
...
impl<T: Sample> SoundReader<T> for ForeignSoundReader<T> {
            ...
            Some(x) => {
                let ret = unsafe {
                    x(
                        self.callback_data,
                        *sample_rate,
                        speaker_layout_to_int(speaker_layout),
                    )
                };
                if ret.is_null() {
    ...
    fn estimate_len(&mut self) -> Option<u64> {
        match self
            ...
            .map(|x| unsafe { x(self.callback_data) })
        {
            None => None,
            Some(u64::MAX) => None,
            Some(x) => Some(x),
...
unsafe extern "C" fn SMS_FormattedSoundStream_new(
    ...
    let read_handler = read_handler
        .expect("SMS_FormattedSoundStream_new: read_handler was NULL!");
    let speaker_layout = speaker_layout_from_int(speaker_layout).expect(
        "SMS_FormattedSoundStream_new: speaker_layout was not a valid \
                 SMS_SPEAKER_LAYOUT_* constant",
    );
    macro_rules! reader {
        ($enum:ident, $type:ident) => {
            FormattedSoundReader::$enum(Box::new(
                ForeignSoundReader::<$type> {
                    callback_data,
                    read_handler,
                    free_handler,
                    seek_handler,
    ...
    Box::into_raw(Box::new(FormattedSoundStream {
        ...
        reader: match format {
            ...
            _ => panic!(
                "SMS_FormattedSoundStream_new: format was not a \
...
#[no_mangle]
unsafe extern "C" fn SMS_FormattedSoundStream_free(
    p: *mut FormattedSoundStream,
) {
