...
#[no_mangle]
unsafe extern "C" fn SMS_FlowControlResponse_free(
...
#[no_mangle]
unsafe extern "C" fn SMS_FlowControlResponse_poll(
    p: *mut query::Response<Option<StringOrNumber>>,
) -> c_int {
    if p.is_null() {
...
unsafe extern "C" fn SMS_FlowControlResponse_get(
    ...
    match responder.try_get() {
        Ok(Some(response)) => match response {
            ...
            StringOrNumber::String(str) => is_string.map(|callback| {
                let first_char_ptr = if str.is_empty() {
                    null()
                } else {
                    &str.as_bytes()[0]
                };
