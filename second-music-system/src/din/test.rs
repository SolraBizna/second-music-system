...
fn iterate_din_parser() {
    let collected: Result<Vec<ParseItem>, _> = DinParser::new(TEST_DOCUMENT)
        .collect();
...
fn parse_din_nodes() {
    assert_eq!(
        ...
        &[
            node!(1, ["formidable", "azure"], [
                node!(2, ["approaching", "storm"]),
                node!(3, ["bringer", "of", "death"]),
            ]),
            node!(5, ["cold-blooded", "argent"], [
                node!(6, ["unbendable", "backbone"], [
                    node!(7, ["treasury", "of", "the", "forsaken", "kingdom"]),
                    node!(8, ["patron", "of", "the", "takers"]),
                ]),
                node!(9, ["strengthen", "thy", "servant"], [
                    node!(10, ["the", "one", "who", "listens", "to", "thee"], [
                        node!(11, ["searches", "for", "thee"]),
                        node!(12, ["and", "is", "blindly", "obedient"]),
                    ]),
                    node!(13, ["shall", "be", "answered"]),
                ]),
            ]),
            node!(15, ["amber"], [
                node!(16, ["rapture", "of", "wilderness"], [
                    node!(17, ["chaos", "of", "maelstroms"], [
                        node!(18, ["banner of the righteous"], [
