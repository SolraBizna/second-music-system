...
impl<T: Sample> FadeAdapter<T> {
    fn new_boxed(
        ...
        if let Some(how_long_to_play_before_fade) =
            how_long_to_play_before_fade
        {
            samples_till_fade_out =
                (how_long_to_play_before_fade * sample_rate).floor() as u64
                    * num_channels;
            samples_left = (((how_long_to_play_before_fade
                + fade_out.max(PosFloat::ZERO))
                * sample_rate)
                .ceil() as u64
                * num_channels)
        ...
        Box::new(FadeAdapter {
            ...
            samples_till_fade_out,
            samples_left,
            fade_in: Fader::maybe_start(
                FadeType::Linear,
                PosFloat::ZERO,
                PosFloat::ONE,
                fade_in,
            ),
            fade_out: Fader::maybe_start(
                FadeType::Linear,
                PosFloat::ONE,
                PosFloat::ZERO,
                fade_out,
...
impl<T: Sample> SoundReader<f32> for FadeAdapter<T> {
    fn read(&mut self, out: &mut [MaybeUninit<f32>]) -> usize {
        ...
        let amount_read =
            self.source_stream.read(&mut self.buf[..amount_to_read]);
        ...
        out[..amount_read]
            .iter_mut()
            .zip(self.buf[..amount_read].iter())
            .for_each(|(o, i)| {
                *o = MaybeUninit::new(
                    unsafe { i.assume_init_ref() }.to_float_sample(),
                );
            });
    ...
    fn skip_coarse(
        ...
        if result > 0 {
            self.samples_till_fade_out =
                self.samples_till_fade_out.saturating_sub(result);
    ...
    fn skip_precise(
        ...
        self.samples_till_fade_out =
            self.samples_till_fade_out.saturating_sub(count);
    ...
    fn estimate_len(&mut self) -> Option<u64> {
        panic!(
            "SMS logic error: attempt to estimate length of a loop adapter"
        );
    }
}
...
pub(crate) fn new_fade_adapter(
    sound: &Sound,
    stream: FormattedSoundStream,
    fade_in: PosFloat,
    length: Option<PosFloat>,
    fade_out: PosFloat,
) -> Box<dyn SoundReader<f32>> {
    let FormattedSoundStream {
        sample_rate,
        speaker_layout,
        reader,
    } = stream;
    match reader {
        FormattedSoundReader::U8(x) => FadeAdapter::new_boxed(
            sound,
            fade_in,
            length,
            fade_out,
        ...
        FormattedSoundReader::U16(x) => FadeAdapter::new_boxed(
            sound,
            fade_in,
            length,
            fade_out,
        ...
        FormattedSoundReader::I8(x) => FadeAdapter::new_boxed(
            sound,
            fade_in,
            length,
            fade_out,
        ...
        FormattedSoundReader::I16(x) => FadeAdapter::new_boxed(
            sound,
            fade_in,
            length,
            fade_out,
        ...
        FormattedSoundReader::F32(x) => FadeAdapter::new_boxed(
            sound,
            fade_in,
            length,
            fade_out,
