...
pub(crate) fn new_rate_adapter(
    ...
    let quality_spec =
        QualitySpec::new(&QualityRecipe::Medium, QualityFlags::HI_PREC_CLOCK);
    ...
    let soxr = match Soxr::create(
        ...
        Err(x) => {
            delegate.warning(&format!(
                "Unable to initialize resampler for {} -> {} Hz: {}",
                in_sample_rate, out_sample_rate, x
            ));
            return in_stream;
        }
    };
...
impl SoundReader<f32> for RateAdapter {
    fn read(&mut self, out: &mut [MaybeUninit<f32>]) -> usize {
        ...
        let in_slice: Option<&[f32]> = if self.fini && in_slice.is_empty() {
            None
        } else {
            unsafe { std::mem::transmute(in_slice) }
        };
        ...
        let (in_consumed, out_produced) = self
            .soxr
            .process(in_slice, out_slice)
            .expect("internal libsoxr error");
