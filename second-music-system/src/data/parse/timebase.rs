...
use crate::{din::DinNode, PosFloat};
...
static DEFAULT_TIMEBASE: Lazy<Timebase> = Lazy::new(|| Timebase {
    stages: vec![TimebaseStage {
        one_based: false,
        multiplier: PosFloat::ONE,
    }],
});
...
impl Timebase {
    pub fn parse_stage(
        mut source: &str,
    ) -> Result<(bool, PosFloat, TimeSpec), String> {
        ...
        let timespec = match end {
            Some(x) => {
                ...
                let res = if let Some(x) = suffix.strip_prefix('/') {
                    ...
                    TIMEBASE_SUFFIX_MAP.get(x).map(|x| TimeSpec::PerSuffix(*x))
                } else {
                    if source.is_empty() {
                        source = "1";
                    }
                ...
                match res {
                    Some(x) => x,
                    None => {
                        return Err(format!("Unknown suffix: {:?}", suffix))
                    }
                }
            }
            None => TimeSpec::Basic,
    ...
    pub fn parse_source(source: &[String]) -> Result<Timebase, String> {
        let mut stages: Vec<(bool, PosFloat)> =
        ...
        let (basis_index, basis_spec) =
            match basis {
                Some(x) => x,
                None => return Err(
                    "This timebase doesn't specify a basis (e.g. \"/minute\")"
                        .to_string(),
                ),
            };
        ...
        for (n, (one_based, mut multiplier)) in &mut iter {
            if n == basis_index {
                match basis_spec {
                    ...
                    TimeSpec::PerSuffix(x) => {
                        multiplier = x.num_per(multiplier)
                    }
                    TimeSpec::TimesSuffix(x) => {
                        multiplier = x.num_times(multiplier)
                    }
                }
            }
...
impl<'a> TimebaseCollection<'a> {
    ...
    pub fn get_timebase(&self, name: &str) -> Option<&Timebase> {
        self.timebases.get(name).or_else(|| {
            if let Some(parent) = self.parent {
                parent.get_timebase(name)
            } else {
                None
            }
        })
    ...
    pub fn get_active_timebase(&self) -> Option<&Timebase> {
        self.active_timebase
            .as_ref()
            .and_then(|x| self.get_timebase(x))
    }
    pub fn parse_timebase_node(
        &mut self,
        node: &DinNode,
    ) -> Result<(), String> {
        match || -> Result<(), String> {
            ...
            let (name, stages) = if node.items[1].starts_with(|x: char| {
                x == '.' || x == '@' || x.is_ascii_digit()
            }) {
    ...
    pub fn parse_time(&self, items: &[String]) -> Result<PosFloat, String> {
        ...
        match timebase.eval(
            time,
            !(items[0].ends_with("length")
                || items[0].starts_with("fade")
                || items[0].starts_with("over")),
        ) {
            Ok(x) => Ok(x),
            Err(x) => Err(x),
        }
    ...
    pub fn parse_time_node(&self, node: &DinNode) -> Result<PosFloat, String> {
        if !node.children.is_empty() {
            return Err(format!(
                "{:?} elements must have no children (check indentation)",
                node.items[0]
            ));
        }
    ...
    assert_eq!(
        ...
        Ok(Timebase {
            stages: vec![
                TimebaseStage {
                    one_based: true,
                    multiplier: PosFloat::new_clamped(2.0)
                },
                TimebaseStage {
                    one_based: false,
                    multiplier: PosFloat::new_clamped(0.5)
                },
                TimebaseStage {
                    one_based: false,
                    multiplier: PosFloat::new_clamped(1.0 / 64.0)
                }
            ]
        })
    );
